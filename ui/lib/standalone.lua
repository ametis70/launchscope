...
function M.launch(app)
    if _pid then return false, "an app is already running" end

    local cmd     = buildCmd(app)
    local wrapped = "sh -c " .. shellQuote(cmd .. " & echo $!")
    local fh      = io.popen(wrapped)
    if not fh then return false, "failed to spawn process" end

    local pid_str = fh:read("*l"); fh:close()
...
function M.stop()
    if not _pid then return end
    os.execute("kill " .. _pid .. " 2>/dev/null")
end

function M.isRunning()  return _pid ~= nil end
function M.currentApp() return _app end
function M.currentPid() return _pid end

function M.update(dt)
    if not _pid then return end
    _poll_timer = _poll_timer + dt
    if _poll_timer < POLL_RATE then return end
    _poll_timer = 0
    ...
    if alive ~= 0 and alive ~= true then
        local old_app = _app
        _pid = nil; _app = nil
        if M.on_exit then M.on_exit(old_app) end
    end
end
