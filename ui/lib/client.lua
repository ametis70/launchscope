...
function M.post(path, payload)
    ...
    if code == 200 or code == 202 then
        if #body > 0 then
            local ok, result = pcall(json.decode, body)
            if ok then return result, code end
        end
        return nil, code
    end
    return nil, code
...
function M.getApps()    return M.get("/api/apps")   end
function M.getStatus()  return M.get("/api/status") end
function M.getConfig()  return M.get("/api/config") end

function M.launch(id)       return M.post("/api/launch/" .. id)                end
